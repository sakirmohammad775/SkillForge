  ...
  if (!course) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white p-6">
        ...
        <Link
          href="/courses"
          className="text-pink-500 font-bold text-[10px] uppercase tracking-widest hover:text-white transition-colors"
        >
          ← Back to Academy
        </Link>
      </div>
                ...
                {course.title.split(" ").map((word, i) => (
                  <span
                    ...
                    className={
                      i === 0
                        ? "text-white"
                        : "bg-gradient-to-r from-pink-500 to-orange-400 bg-clip-text text-transparent ml-3"
                    }
                  >
                    {word}
                  </span>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-10 bg-[#0f0f0f] border border-white/10 rounded-[2.5rem] p-10 space-y-8 shadow-2xl shadow-pink-500/5">
              ...
              <div className="space-y-4">
                <button
                  onClick={() => router.push(`/billing/${course.id}`)}
                  className="w-full bg-white text-black font-black py-5 rounded-2xl hover:bg-gray-200 active:scale-[0.98] transition-all duration-300 flex items-center justify-center gap-3 text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-white/5"
                >
